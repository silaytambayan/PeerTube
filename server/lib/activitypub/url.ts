...
function getVideoCacheFileActivityPubUrl (video: VideoModel, videoFile: VideoFileModel) {
  const suffixFPS = videoFile.fps && videoFile.fps !== -1 ? '-' + videoFile.fps : ''

  return `${CONFIG.WEBSERVER.URL}/redundancy/videos/${video.uuid}/${videoFile.resolution}${suffixFPS}`
}

function getVideoCacheStreamingPlaylistActivityPubUrl (playlist: VideoStreamingPlaylistModel) {
  return `${CONFIG.WEBSERVER.URL}/redundancy/video-playlists/${playlist.getStringType()}/${playlist.Video.uuid}`
}
