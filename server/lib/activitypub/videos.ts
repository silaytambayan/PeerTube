...
function streamingPlaylistActivityUrlToDBAttributes (video: VideoModel, videoObject: VideoTorrentObject) {
  ...
  const attributes: VideoStreamingPlaylistModel[] = []
  for (const playlistUrlObject of playlistUrls) {
    ...
    const segmentsSha256UrlObject = playlistUrlObject.tag
                                                     .find(t => isAPPlaylistSegmentHashesUrlObject(t)) as ActivityPlaylistSegmentHashesObject
    ...
    const attribute = {
      ...
      videoId: video.id,
    } as VideoStreamingPlaylistModel
